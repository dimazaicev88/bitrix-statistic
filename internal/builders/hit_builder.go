...
var mapFields = map[string]string{
	...
	"CITY_ID":            "t1.CITY_ID ",
	"REGION REGION_NAME": "t2.REGION ",
	"NAME CITY_NAME":     "t2.CITY_NAME ",
	"SITE_ID":            "t1.SITE_ID ",
}
...
func (hs HitSQLBuilder) buildSelectAndJoin() HitSQLBuilder {
	...
	if len(hs.filter.Select) == 0 {
		...
		for _, selectField := range hs.filter.Select {
			if value, ok := mapFields[selectField]; ok {
				hs.selectBuilder.WriteString(value)
			}
			if selectField == "USER" {
...
func (hs HitSQLBuilder) orderByBuild() HitSQLBuilder {
	...
	for _, by := range hs.filter.OrderBy {
		orderByFields = append(orderByFields, mapFields[by])
	}
