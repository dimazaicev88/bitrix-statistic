...
func TestCountryCase1(t *testing.T) {
	...
	jsonStr := `{"SELECT":["SHORT_NAME"],"WHERE":"ID = :countryId","PARAMS":{":countryId":1},"ORDER_BY":["CountryId","ID"],"TYPE_SORT":"ASC"}`
	jsoniter.Unmarshal([]byte(jsonStr), &filter)
	sql, _ := NewCountrySQLBuilder(filter).BuildSQL()
	assert.Equal(t, "SELECT t1.SHORT_NAME FROM b_stat_country t1  WHERE  t1.ID =  ?  ORDER BY  t1.CountryId , t1.ID ASC", sql.SQL)
