...
func __SetReferer(s string, s2 string) {
...
func (s Statistic) RestoreSession(phpServer entity.PhpServer, cookieGuestId int, session *session.Session, sessionGcMaxLifeTime string) {
	...
	sessionId, err := session.GetAsInt("SESS_SESSION_ID")
	if session.KeyExists("SESS_SESSION_ID") == false || (err != nil && sessionId <= 0) {
		if session.Get("SAVE_SESSION_DATA") == "Y" {
			...
			if cookieGuestId <= 0 {
				...
				sess, err := s.sessionDataModel.FindSessionByGuestMd5(md5, sessionGcMaxLifeTime)
				session.SetAll(&sess)
			}
		}
	}
