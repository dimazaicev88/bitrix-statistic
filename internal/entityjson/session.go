...
type Session struct {
	Uuid         uuid.UUID `json:"uuid"`                  // Uuid сессии
	...
	StopListUuid uuid.UUID `json:"stopListId,omitempty"`  // ID записи стоп-листа, под которую попал посетитель (если это имело место быть)
	...
	FirstHitUuid uuid.UUID `json:"firstHitId,omitempty"`  // ID первого хита
	...
	LastHitUuid  uuid.UUID `json:"lastHitId,omitempty"`   // ID последнего хита
	...
	AdvUuid      uuid.UUID `json:"advId,omitempty"`       // ID рекламной кампании
